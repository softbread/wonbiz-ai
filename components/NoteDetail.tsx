...
const NoteDetail: React.FC<NoteDetailProps> = ({ note, llmConfig, onBack, onDelete, language = 'en' }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-wonbiz-black animate-fade-in">
      {/* Header */}
      <div className="flex items-center px-6 py-4 border-b border-wonbiz-gray bg-wonbiz-dark/50 backdrop-blur-md sticky top-0 z-10">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-wonbiz-gray font-mono">{new Date(note.createdAt).toLocaleString()} • {formatDuration(note.duration)}</p>
        </div>
      ...
      <div className="bg-wonbiz-dark border-b border-wonbiz-gray px-6 py-3 flex items-center gap-4">
        {hasAudio ? (
          <>
            ...
            <span className="text-xs font-mono tabular-nums text-wonbiz-gray">{formatDuration(currentTime)} / {formatDuration(note.duration)}</span>
          </>
        ) : (
